...
import { MarqueeTape, ThemeToggle } from '../components/ui';
...
const FEATURES = [
  ...
  { icon: Trophy,      title: 'XP & Analytics',    desc: 'Leaderboards, badges, and spending insights. 📊', color: 'bcard-aqua' },
];

const TESTIMONIALS = [
  { text: '"Splitwise se zyada fun hai bhai 💀"', name: 'Rahul M., Mumbai', sticker: 'bro verified' },
  { text: '"Analytics dekh ke pata chala main hi squad ka sabse bada kharchi hoon."', name: 'Priya K., Delhi', sticker: 'paisa clear' },
...
export default function Landing() {
  ...
  return (
    <main className="min-h-screen" style={{ background: 'var(--color-ink-950)' }}>
      {/* NAV */}
      <nav className="mx-auto flex max-w-6xl items-center justify-between px-5 py-5">
        <span className="font-display text-xl font-extrabold" style={{ color: 'var(--color-bone)' }}>
          Squad<span style={{ color: 'var(--color-marigold)' }}>Pay</span>
      ...
      <section className="mx-auto max-w-5xl px-5 pt-10 pb-12 sm:py-20">
        <div className="grid items-center gap-10 lg:grid-cols-2">
          <div>
            ...
            <div className="min-h-[8rem] sm:min-h-[10rem] overflow-hidden">
              <AnimatePresence mode="wait">
                <motion.h1 key={hi} initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -40, opacity: 0 }}
                  ...
                  className="font-display text-4xl font-extrabold leading-[1.1] sm:text-5xl lg:text-6xl" style={{ color: 'var(--color-bone)' }}>
                  {HERO[hi].split('\n').map((line, i) => (
                    <span key={i}>{i === 1 ? <span style={{ color: 'var(--color-marigold)' }}>{line}</span> : line}<br /></span>
                  ))}
                </motion.h1>
              </AnimatePresence>
            </div>
            <p className="mt-4 max-w-md text-base" style={{ color: 'rgba(var(--rt-bone-rgb),0.6)' }}>
            ...
            {/* Rotating tagline */}
            <p className="mt-5 text-sm italic" style={{ color: 'var(--color-marigold)' }}>
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }}
            ...
            <div className="mb-3 rounded-xl bg-ink-800 p-4">
              <p className="text-xs font-bold uppercase tracking-wider" style={{ color: 'rgba(var(--rt-bone-rgb),0.5)' }}>Squad spend</p>
              <p className="font-display text-3xl font-extrabold" style={{ color: 'var(--color-marigold)' }}>₹12,400</p>
            </div>
            {[
              ...
              <div key={m.name} className="flex justify-between py-2 text-sm" style={{ borderBottom: '1px solid rgba(var(--rt-bone-rgb),0.1)' }}>
                <span style={{ color: 'var(--color-bone)' }}>{m.name}</span>
                <b style={{ color: m.green ? 'var(--color-mint)' : 'var(--color-rose)' }}>{m.net}</b>
              </div>
            ))}
      ...
      <section className="mx-auto max-w-5xl px-5 pb-16">
        <div className="mb-8 flex items-center gap-4">
          <h2 className="font-display text-2xl font-extrabold" style={{ color: 'var(--color-bone)' }}>Kya kya milega?</h2>
          <div style={{ flex: 1, height: 2, background: 'rgba(var(--rt-bone-rgb),0.1)' }} />
        </div>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {FEATURES.map((f, i) => (
            <motion.div key={f.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}>
              <div className={`bcard ${f.color} h-full p-5`}>
                <f.icon className="h-7 w-7 mb-3" style={{ color: 'var(--color-marigold)' }} />
                <h3 className="font-display text-sm font-extrabold mb-1" style={{ color: 'var(--color-bone)' }}>{f.title}</h3>
                <p className="text-xs leading-relaxed" style={{ color: 'rgba(var(--rt-bone-rgb),0.6)' }}>{f.desc}</p>
              </div>
            </motion.div>
          ))}
        </div>
      ...
      {/* TESTIMONIALS */}
      <section style={{ borderTop: '2px solid rgba(var(--rt-bone-rgb),0.1)' }} className="mx-auto max-w-5xl px-5 py-14">
        <h2 className="mb-6 font-display text-2xl font-extrabold text-center" style={{ color: 'var(--color-bone)' }}>Squad ki reviews 😂</h2>
        <div className="grid gap-4 sm:grid-cols-3">
          {TESTIMONIALS.map((t, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
              <div className={`bcard ${['bcard-yellow','bcard-pink','bcard-lime'][i]} p-5 h-full`}>
                <p className="text-sm leading-relaxed mb-3" style={{ color: 'var(--color-bone)' }}>{t.text}</p>
                <p className="text-xs" style={{ color: 'rgba(var(--rt-bone-rgb),0.5)' }}>— {t.name}</p>
      ...
      {/* FOOTER CTA */}
      <MarqueeTape />
      <footer className="py-10 text-center" style={{ borderTop: '2px solid rgba(var(--rt-bone-rgb),0.1)' }}>
        <p className="font-display text-sm font-bold" style={{ color: 'rgba(var(--rt-bone-rgb),0.4)' }}>
