  ...
  return (
    <main className="min-h-screen pb-28" style={{ background:'var(--bg)' }}>
      ...
      <nav className="mx-auto flex max-w-5xl items-center justify-between px-5 py-5 relative">
        <button onClick={() => nav(`/app/squad/${id}/trips`)} className="flex items-center gap-2 text-sm font-bold" style={{ color:'var(--text-dim)' }}>
          <ArrowLeft className="h-4 w-4" /> Trips
        </button>
        <button onClick={() => setShowMenu(!showMenu)} className="rounded-lg p-2" style={{ color:'var(--text-dim)' }}>
          <MoreVertical className="h-5 w-5" />
        </button>
        <AnimatePresence>
          {showMenu && (
            <motion.div initial={{ opacity:0, y:-8 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0 }}
              className="absolute right-5 top-14 z-30 w-48 rounded-2xl p-2" style={{ background:'var(--card)', border:'2px solid var(--border)' }}>
      ...
      <header className="mx-auto max-w-5xl px-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display text-2xl font-extrabold sm:text-3xl" style={{ color:'var(--text)' }}>{trip.name}</h1>
        ...
        <div className="mt-5 grid grid-cols-2 gap-3">
          <div className="bcard bcard-yellow p-4" style={{ background:'rgba(245,166,35,0.08)' }}>
            <p className="text-[10px] font-bold uppercase tracking-wider" style={{ color:'var(--text-dim)' }}>Total Spend</p>
          ...
          <div className="bcard bcard-aqua p-4" style={{ background:'rgba(0,212,200,0.08)' }}>
            <p className="text-[10px] font-bold uppercase tracking-wider" style={{ color:'var(--text-dim)' }}>Budget</p>
            {trip.budget ? (
              <>
                <p className="font-display text-xl font-extrabold mt-0.5" style={{ color:budgetColor }}>{trip.budgetUsedPct}%</p>
                <div className="mt-1.5 h-1.5 rounded-full overflow-hidden" style={{ background:'var(--border-ghost)' }}>
            ...
            ) : <p className="font-display text-sm font-bold mt-0.5" style={{ color:'var(--text-faint)' }}>Not set</p>}
          </div>
        </div>
        ...
        {balances.some(b => b.net !== 0) && (
          <div className="bcard mt-4 p-4">
            <p className="font-display text-sm font-extrabold mb-3" style={{ color:'var(--text)' }}>Trip Balances</p>
        ...
        <section className="mx-auto max-w-5xl px-5 mt-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            {leaderboard.biggestSpender && (
              <div className="bcard bcard-yellow p-4" style={{ background:'rgba(245,166,35,0.08)' }}>
                <p className="text-[10px] font-bold uppercase tracking-wider mb-2" style={{ color:'var(--text-dim)' }}>👑 Biggest Spender</p>
                <div className="flex items-center gap-2">
                  <Avatar url={leaderboard.biggestSpender.avatarUrl} name={leaderboard.biggestSpender.name} size="h-8 w-8" />
                  <div>
                    <p className="font-display text-sm font-extrabold" style={{ color:'var(--text)' }}>{leaderboard.biggestSpender.name.split(' ')[0]}</p>
            ...
            {leaderboard.mostActive && (
              <div className="bcard bcard-lime p-4" style={{ background:'rgba(184,240,42,0.08)' }}>
                <p className="text-[10px] font-bold uppercase tracking-wider mb-2" style={{ color:'var(--text-dim)' }}>⚡ Most Active</p>
                <div className="flex items-center gap-2">
                  <Avatar url={leaderboard.mostActive.avatarUrl} name={leaderboard.mostActive.name} size="h-8 w-8" />
                  <div>
                    <p className="font-display text-sm font-extrabold" style={{ color:'var(--text)' }}>{leaderboard.mostActive.name.split(' ')[0]}</p>
            ...
            {leaderboard.topPhotographer && (
              <div className="bcard bcard-pink p-4" style={{ background:'rgba(255,61,110,0.08)' }}>
                <p className="text-[10px] font-bold uppercase tracking-wider mb-2" style={{ color:'var(--text-dim)' }}>📸 Top Photographer</p>
                <div className="flex items-center gap-2">
                  <Avatar url={leaderboard.topPhotographer.avatarUrl} name={leaderboard.topPhotographer.name} size="h-8 w-8" />
                  <div>
                    <p className="font-display text-sm font-extrabold" style={{ color:'var(--text)' }}>{leaderboard.topPhotographer.name.split(' ')[0]}</p>
      ...
      <section className="mx-auto max-w-5xl px-5 mt-6">
        {expenses.length === 0 ? (
          <div className="bcard p-8 text-center">
            ...
            <p style={{ color:'var(--text-faint)' }}>Abhi tak koi kharcha is trip mein nahi. Squad page se expense add karte waqt trip select karo.</p>
            ...
            <div className="space-y-2">
              {expenses.map((e) => (
                <div key={e.id} className="bcard flex items-center gap-3 p-4">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-lg" style={{ background:'var(--border-ghost)' }}>{CE[e.category]}</div>
                  <div className="min-w-0 flex-1">
                    <p className="truncate font-display font-bold text-sm" style={{ color:'var(--text)' }}>{e.title}</p>
                    <p className="text-xs" style={{ color:'var(--text-dim)' }}>{e.paid_by_name.split(' ')[0]} ne diya</p>
                  </div>
        ...
        {photos.length > 0 && (
          <div className="mt-6">
            ...
            <div className="grid grid-cols-3 gap-2">
              {photos.map((p) => (
                <img key={p.id} src={p.url} alt={p.caption||'memory'} className="aspect-square w-full rounded-xl object-cover" style={{ border:'2px solid var(--border-ghost)' }} />
              ))}
            </div>
          </div>
        )}
        ...
        {showTagModal && (
          <>
            ...
            <motion.div initial={{ y:'100%' }} animate={{ y:0 }} exit={{ y:'100%' }}
              ...
              style={{ background:'var(--card)', border:'2px solid var(--border)', borderBottom:'none', maxHeight:'80vh', overflowY:'auto' }}>
              <div className="flex justify-center pt-3 pb-1">
                <div className="h-1 w-10 rounded-full" style={{ background:'var(--border-strong)' }} />
              </div>
              <div className="flex items-center justify-between px-5 pt-2 pb-4">
                <div>
                  <h2 className="font-display text-lg font-extrabold" style={{ color:'var(--text)' }}>Purane kharche tag karo</h2>
                  <p className="text-xs" style={{ color:'var(--text-dim)' }}>Squad ke saare untagged expenses yahan hain</p>
                </div>
                <button onClick={() => setShowTagModal(false)} className="rounded-full p-2" style={{ background:'var(--border-ghost)', color:'var(--text-dim)' }}>
                  <X className="h-4 w-4" />
                </button>
              </div>
              ...
              <div className="px-5 pb-8 space-y-2">
                {!unassigned ? (
                  ...
                  <div className="py-10 text-center">
                    <p className="text-3xl mb-2">✅</p>
                    <p className="text-sm" style={{ color:'var(--text-faint)' }}>Sab kharche already kisi trip mein tagged hain!</p>
                  </div>
                ) : unassigned.map((e) => (
                  <div key={e.id} className="flex items-center gap-3 rounded-2xl p-3" style={{ background:'var(--border-ghost)', border:'2px solid var(--border-ghost)' }}>
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-lg" style={{ background:'var(--border-ghost)' }}>{CE[e.category]||'📦'}</div>
                    <div className="min-w-0 flex-1">
                      <p className="truncate font-display font-bold text-sm" style={{ color:'var(--text)' }}>{e.title}</p>
                      <p className="text-[11px]" style={{ color:'var(--text-dim)' }}>{e.paid_by_name.split(' ')[0]} · {new Date(e.expense_date).toLocaleDateString('en-IN',{day:'numeric',month:'short'})}</p>
                    </div>
