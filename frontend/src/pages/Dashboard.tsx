    ...
    <main className="min-h-screen" style={{ background:'var(--bg)' }}>
      ...
      <nav className="mx-auto flex max-w-5xl items-center justify-between px-5 py-5">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button onClick={() => { logout(); nav('/'); }} className="rounded-lg p-2 transition hover:bg-white/10" style={{ color:'var(--text-faint)' }} title="Logout">
      ...
      <section className="mx-auto max-w-5xl px-5 pb-20">
        ...
        {!squads ? <FunLoader /> : squads.length === 0 ? (
          <div className="bcard bcard-yellow p-12 text-center">
            ...
            <h2 className="mt-4 font-display text-lg font-extrabold" style={{ color:'var(--text)' }}>Koi squad nahi? Akele kharcha karoge? 😭</h2>
            <p className="mx-auto mt-2 max-w-xs text-sm" style={{ color:'var(--text-dim)' }}>Squad banao ya kisi dost ka invite code maango — chaos shuru karte hain.</p>
              ...
              <motion.div key={s.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}>
                <Link to={`/app/squad/${s.id}`}>
                  <div className={`bcard ${['bcard-yellow','bcard-pink','bcard-lime','bcard-aqua'][i % 4]} h-full p-5 transition hover:translate-x-[-2px] hover:translate-y-[-2px]`}>
                    ...
                    <h3 className="font-display font-extrabold" style={{ color:'var(--text)' }}>{s.name}</h3>
                    <div className="mt-3 flex items-center justify-between text-xs" style={{ color:'var(--text-dim)' }}>
      ...
      <Modal open={modal === 'create'} onClose={() => setModal(null)} title="Squad banao 🎉">
        <form onSubmit={create} className="space-y-4">
          ...
          <label className="block">
            <span className="mb-1.5 block text-xs font-bold uppercase tracking-wider" style={{ color:'var(--text-dim)' }}>Vibe</span>
