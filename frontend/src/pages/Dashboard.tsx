...
import { Button, Input, Modal, ErrorText, Avatar, FunLoader, MarqueeTape, SoundToggle, ThemeToggle } from '../components/ui';
      ...
      <section className="mx-auto max-w-5xl px-5 pb-20">
        <div className="flex flex-wrap items-start justify-between gap-4 mb-8">
          <div>
            ...
            <p className="mt-1 text-sm" style={{ color: 'rgba(var(--rt-bone-rgb),0.5)' }}>
        ...
        {!squads ? <FunLoader /> : squads.length === 0 ? (
          <div className="bcard bcard-yellow p-12 text-center">
            ...
            <h2 className="mt-4 font-display text-lg font-extrabold" style={{ color: 'var(--color-bone)' }}>Koi squad nahi? Akele kharcha karoge? 😭</h2>
            <p className="mx-auto mt-2 max-w-xs text-sm" style={{ color: 'rgba(var(--rt-bone-rgb),0.5)' }}>Squad banao ya kisi dost ka invite code maango — chaos shuru karte hain.</p>
              ...
              <motion.div key={s.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}>
                <Link to={`/app/squad/${s.id}`}>
                  <div className={`bcard ${['bcard-yellow','bcard-pink','bcard-lime','bcard-aqua'][i % 4]} h-full p-5 transition hover:translate-x-[-2px] hover:translate-y-[-2px]`}>
                    ...
                    <h3 className="font-display font-extrabold" style={{ color: 'var(--color-bone)' }}>{s.name}</h3>
                    <div className="mt-3 flex items-center justify-between text-xs" style={{ color: 'rgba(var(--rt-bone-rgb),0.5)' }}>
                      <span>{s.member_count} member{Number(s.member_count) !== 1 && 's'}</span>
                      <span className="font-display font-bold" style={{ color: 'var(--color-marigold)' }}>{toRupees(Number(s.total_spend))}</span>
                    </div>
                  </div>
                </Link>
              </motion.div>
