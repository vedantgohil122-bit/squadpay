              ...
              <div className="space-y-2">
                {history.length === 0 ? (
                  <div className="bcard p-10 text-center">
                    ...
                    <p className="mt-3 font-display font-bold" style={{ color:'var(--color-bone)' }}>Koi history nahi abhi</p>
                    <p className="mt-1 text-sm" style={{ color:'rgba(var(--rt-bone-rgb),0.4)' }}>Contribution add karo to history yahan dikhegi</p>
                  </div>
                ) : history.map((h, i) => (
                  <motion.div key={h.id} initial={{ opacity:0, x:-12 }} animate={{ opacity:1, x:0 }} transition={{ delay:i*0.04 }}>
                    <div className="bcard flex items-center gap-3 p-4">
                      ...
                      <p className="font-display font-extrabold shrink-0" style={{ color: TYPE_STYLE[h.type]?.color || 'var(--color-bone)' }}>
                        {h.type==='deposit'?'+':'-'}{toRupees(Number(h.amount))}
                      </p>
                    </div>
                  </motion.div>
