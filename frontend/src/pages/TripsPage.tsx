  ...
  return (
    <main className="min-h-screen pb-24" style={{ background:'var(--bg)' }}>
      ...
      <nav className="mx-auto flex max-w-5xl items-center justify-between px-5 py-5">
        ...
        <h1 className="font-display font-extrabold" style={{ color:'var(--text)' }}>🧳 Trips</h1>
      </nav>
      ...
      <section className="mx-auto max-w-5xl px-5">
        {trips.length === 0 ? (
          <div className="bcard bcard-yellow p-12 text-center">
            ...
            <h2 className="mt-4 font-display text-lg font-extrabold" style={{ color:'var(--text)' }}>Koi trip nahi abhi tak!</h2>
            <p className="mx-auto mt-2 max-w-xs text-sm" style={{ color:'var(--text-dim)' }}>Goa, Manali, ya simple weekend outing — sab trip mein group karo.</p>
...
function TripCard({ trip, idx, squadId, faded }: { trip: Trip; idx: number; squadId: string; faded?: boolean }) {
  ...
  return (
    <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity: faded?0.6:1, y:0 }} transition={{ delay: idx*0.06 }}
      ...
      <h3 className="font-display font-extrabold" style={{ color:'var(--text)' }}>{trip.name}</h3>
