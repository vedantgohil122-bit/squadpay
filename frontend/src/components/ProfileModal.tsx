  ...
  return (
    <Modal open={open} onClose={onClose} title="Apna look set karo ✨">
      <form onSubmit={save} className="max-h-[75vh] overflow-y-auto pr-1 space-y-4">
          ...
          <div className="relative">
            {avatar ? (
              <img src={avatar} alt="avatar" className="h-20 w-20 rounded-full border-4"
                style={{ borderColor: 'var(--color-marigold)', background: 'var(--color-ink-900)' }} />
            ) : (
              <div className="h-20 w-20 rounded-full border-4 flex items-center justify-center font-display font-extrabold text-2xl"
                style={{ borderColor: 'var(--color-marigold)', background: 'var(--color-ink-900)', color: 'var(--color-marigold)' }}>
                {name[0] || '?'}
              </div>
            )}
            <button type="button" onClick={() => fileRef.current?.click()}
              className="absolute -bottom-1 -right-1 rounded-full p-1.5"
              style={{ background: 'var(--color-marigold)', border: '2px solid #0e0c0a' }}>
              <Camera size={12} color="var(--color-ink-950)" />
            </button>
            <input ref={fileRef} type="file" accept="image/*" className="hidden"
              onChange={(e) => { handleFileChange(e.target.files?.[0] || null); setTab('upload'); }} />
          </div>
          <p style={{ fontSize: '0.7rem', color: 'rgba(var(--rt-bone-rgb),0.4)' }}>Tap camera to upload photo</p>
        </div>
        ...
        <div className="flex gap-1 rounded-xl p-1" style={{ background: 'rgba(var(--rt-bone-rgb),0.05)', border: '2px solid rgba(var(--rt-bone-rgb),0.1)' }}>
          {TABS.map(({ id, label, icon: Icon }) => (
            <button key={id} type="button" onClick={() => setTab(id)}
              ...
              style={{ background: tab === id ? 'var(--color-marigold)' : 'transparent', color: tab === id ? 'var(--color-ink-950)' : 'rgba(var(--rt-bone-rgb),0.5)' }}>
              <Icon size={12} /> {label}
            </button>
          ))}
        </div>
        ...
        {tab === 'styles' && (
          <div>
            ...
            <div className="flex flex-wrap gap-1.5 mb-3">
              {STYLES.map((s) => (
                <button key={s.id} type="button" onClick={() => setSelectedStyle(s.id)}
                  ...
                  style={{
                    background: selectedStyle === s.id ? 'rgba(245,166,35,0.2)' : 'rgba(var(--rt-bone-rgb),0.05)',
                    borderColor: selectedStyle === s.id ? 'var(--color-marigold)' : 'rgba(var(--rt-bone-rgb),0.15)',
                    color: selectedStyle === s.id ? 'var(--color-marigold)' : 'rgba(var(--rt-bone-rgb),0.6)',
                  }}>
                  {s.emoji} {s.label}
                </button>
              ))}
              ...
              {avatarOptions.map((url) => (
                <button key={url} type="button" onClick={() => setAvatar(url)}
                  ...
                  style={{
                    ...
                    borderColor: avatar === url ? 'var(--color-marigold)' : 'rgba(var(--rt-bone-rgb),0.1)',
            ...
            <button type="button" onClick={() => setSalt((s) => s + 1)}
              ...
              style={{ background: 'rgba(var(--rt-bone-rgb),0.05)', borderColor: 'rgba(var(--rt-bone-rgb),0.15)', color: 'rgba(var(--rt-bone-rgb),0.6)' }}>
        ...
        {tab === 'upload' && (
          <div>
            <button type="button" onClick={() => fileRef.current?.click()}
              ...
              style={{ borderColor: uploadPreview ? 'var(--color-marigold)' : 'rgba(var(--rt-bone-rgb),0.2)', background: 'rgba(var(--rt-bone-rgb),0.03)' }}>
              {uploadPreview ? (
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p style={{ fontSize: '0.75rem', color: 'var(--color-marigold)', fontWeight: 700 }}>Photo ready! ✅</p>
                  <p style={{ fontSize: '0.65rem', color: 'rgba(var(--rt-bone-rgb),0.4)' }}>Tap to change</p>
                </div>
              ) : (
                <div className="flex flex-col items-center gap-3">
                  <Upload size={32} color="rgba(var(--rt-bone-rgb),0.3)" />
                  <p style={{ fontSize: '0.875rem', fontWeight: 700, color: 'rgba(var(--rt-bone-rgb),0.6)' }}>Apni photo upload karo</p>
                  <p style={{ fontSize: '0.7rem', color: 'rgba(var(--rt-bone-rgb),0.3)' }}>JPG, PNG, WebP · max 2MB</p>
                </div>
              )}
            </button>
            <input ref={fileRef} type="file" accept="image/*" className="hidden"
        ...
        {tab === 'ai' && (
          <div className="space-y-3">
            <div className="rounded-2xl p-4" style={{ background: 'rgba(245,166,35,0.08)', border: '2px solid rgba(245,166,35,0.3)' }}>
              <p style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--color-marigold)', marginBottom: '0.5rem' }}>✨ Describe your vibe</p>
              <p style={{ fontSize: '0.65rem', color: 'rgba(var(--rt-bone-rgb),0.5)', marginBottom: '0.75rem' }}>
              ...
              <div className="flex gap-2">
                ...
                <button type="button" onClick={generateAiAvatar} disabled={aiLoading || !aiPrompt.trim()}
                  ...
                  style={{ background: 'var(--color-marigold)', color: 'var(--color-ink-950)' }}>
                  {aiLoading ? '...' : '✨'}
                </button>
              </div>
            </div>
            ...
            {avatar && tab === 'ai' && (
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                ...
                <img src={avatar} alt="ai avatar" className="h-28 w-28 rounded-full border-4 object-cover"
                  style={{ borderColor: 'var(--color-marigold)', background: 'var(--color-ink-900)' }} />
                <p style={{ fontSize: '0.75rem', color: 'var(--color-marigold)', fontWeight: 700 }}>Generated! ✨</p>
                <button type="button" onClick={generateAiAvatar} className="text-xs underline"
                  style={{ color: 'rgba(var(--rt-bone-rgb),0.4)' }}>Generate another</button>
              </motion.div>
            )}
            ...
            {!avatar && !aiLoading && (
              <div className="py-8 text-center">
                ...
                <p style={{ fontSize: '0.875rem', color: 'rgba(var(--rt-bone-rgb),0.4)' }}>Describe karo, generate ho jayega</p>
              </div>
            )}
          </div>
        )}
        ...
        {/* PROFILE FIELDS */}
        <div style={{ borderTop: '2px solid rgba(var(--rt-bone-rgb),0.08)', paddingTop: '1rem' }} className="space-y-3">
        ...
        <button type="submit" disabled={busy}
          ...
          style={{ background: saved ? 'var(--color-lime)' : 'var(--color-marigold)', color: 'var(--color-ink-950)', border: '2px solid rgba(0,0,0,0.2)' }}>
