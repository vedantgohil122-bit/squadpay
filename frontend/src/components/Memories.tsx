    ...
    <div className="space-y-4">
      ...
      {memories.length === 0 ? (
        <div className="bcard p-10 text-center">
          ...
          <p className="mt-4 font-display font-extrabold" style={{ color: 'var(--color-bone)' }}>Sirf kharche track karoge ya yaadein bhi banaoge?</p>
          <p className="mt-1.5 text-sm" style={{ color: 'rgba(var(--rt-bone-rgb),0.5)' }}>Photos upload karo aur outing ko memory banao.</p>
        </div>
      ) : (
...
function MemoryCard({ m, meId, isAdmin, onReact, onDelete, onChanged }: {
  ...
  return (
    <div className="bcard overflow-hidden">
      {/* PHOTO */}
      <div className="relative" style={{ background: 'var(--color-ink-800)' }}>
        {!imgLoaded && (
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-2.5 mb-2">
          ...
          <p className="flex-1 truncate text-xs font-bold" style={{ color: 'var(--color-bone)' }}>
            {m.uploader_name.split(' ')[0]}
            <span className="ml-1.5 font-normal" style={{ color: 'rgba(var(--rt-bone-rgb),0.4)' }}>· {timeAgo(m.created_at)}</span>
          </p>
        </div>
        {m.caption && <p className="text-sm mb-3" style={{ color: 'var(--color-bone)' }}>{m.caption}</p>}
          ...
          {EMOJIS.map((e) => {
            ...
            return (
              <button key={e} onClick={() => onReact(m.id, e)}
                ...
                style={{
                  background: isMe ? 'rgba(245,166,35,0.2)' : 'rgba(var(--rt-bone-rgb),0.06)',
                  border: isMe ? '2px solid rgba(245,166,35,0.6)' : '2px solid rgba(var(--rt-bone-rgb),0.1)',
                }}>
                {e}
                {count > 0 && <span style={{ fontSize: '0.7rem', color: isMe ? 'var(--color-marigold)' : 'rgba(var(--rt-bone-rgb),0.6)' }}>{count}</span>}
              </button>
            );
          })}
          {totalReactions > 0 && (
            <span className="ml-auto text-[11px]" style={{ color: 'rgba(var(--rt-bone-rgb),0.4)' }}>
        ...
        <button onClick={() => setShowComments(!showComments)}
          ...
          style={{ color: 'rgba(var(--rt-bone-rgb),0.5)' }}>
        ...
        <AnimatePresence>
          {showComments && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
              <div className="pt-2 space-y-2" style={{ borderTop: '2px solid rgba(var(--rt-bone-rgb),0.08)' }}>
                {comments.map((c) => (
                  <p key={c.id} className="text-xs" style={{ color: 'rgba(var(--rt-bone-rgb),0.8)' }}>
                    <b style={{ color: 'var(--color-marigold)' }}>{c.name.split(' ')[0]}</b>
                ...
                <form onSubmit={comment} className="flex gap-2 pt-1">
                  ...
                  <button type="submit" className="rounded-xl px-3 font-bold transition active:scale-90"
                    style={{ background: 'rgba(245,166,35,0.2)', color: 'var(--color-marigold)', border: '2px solid rgba(245,166,35,0.4)' }}>
