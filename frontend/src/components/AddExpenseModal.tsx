    ...
    <Modal open={open} onClose={onClose} title={presetTripId ? `${presetTripLabel || 'Trip'} ka kharcha 🧾` : 'Kharcha add karo 🧾'}>
      <form onSubmit={submit} className="max-h-[70vh] space-y-4 overflow-y-auto pr-1">
        ...
        <label className="block">
          <span className="mb-1.5 block text-xs font-bold uppercase tracking-wider" style={{ color:'var(--text-dim)' }}>Category</span>
        ...
        <label className="block">
          <span className="mb-1.5 block text-xs font-bold uppercase tracking-wider" style={{ color:'var(--text-dim)' }}>Kisne diya?</span>
        ...
        <label className="block">
          <span className="mb-1.5 block text-xs font-bold uppercase tracking-wider" style={{ color:'var(--text-dim)' }}>Split type</span>
          <div className="grid grid-cols-4 gap-1.5">
            {(['equal', 'percentage', 'shares', 'custom'] as const).map((t) => (
              <button key={t} type="button" onClick={() => setForm({ ...form, splitType: t })}
                ...
                style={{ color: form.splitType === t ? '#00d4c8' : 'var(--text-dim)' }}>
                {t}
              </button>
            ))}
          </div>
        ...
        <div className="block">
          <span className="mb-1.5 block text-xs font-bold uppercase tracking-wider" style={{ color:'var(--text-dim)' }}>Kaun kaun mein?</span>
            ...
            {members.map((m) => (
              <div key={m.id} className="flex items-center gap-2">
                <button type="button" onClick={() => setSelected({ ...selected, [m.id]: !selected[m.id] })}
                  ...
                  <span className="truncate" style={{ color:'var(--text)' }}>{m.name.split(' ')[0]}</span>
                </button>
        ...
        {presetTripId ? (
          <div className="rounded-xl px-3 py-2.5 flex items-center justify-between" style={{ background: 'rgba(255,61,110,0.08)', border: '2px solid rgba(255,61,110,0.3)' }}>
            <span className="text-xs font-bold" style={{ color:'var(--text-dim)' }}>🧳 Trip</span>
            <span className="text-sm font-display font-extrabold" style={{ color: '#ff3d6e' }}>{presetTripLabel}</span>
          </div>
        ) : trips.length > 0 && (
          <label className="block">
            <span className="mb-1.5 block text-xs font-bold uppercase tracking-wider" style={{ color:'var(--text-dim)' }}>Trip mein add karo? (optional)</span>
        ...
        <label className="block">
          <span className="mb-1.5 block text-xs font-bold uppercase tracking-wider" style={{ color:'var(--text-dim)' }}>Treasury se? (₹, optional)</span>
          <input type="number" step="0.01" min="0" placeholder="0 (leave empty if not using treasury)"
            ...
            className="binput" />
          <p className="mt-1 text-[10px]" style={{ color:'var(--text-faint)' }}>🏦 Treasury se kitna amount use hoga?</p>
        </label>
